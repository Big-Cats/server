// source: https://raw.githubusercontent.com/davejt/exercise/master/data/exercises

const data = `ab crunch machine, abdominals
...
const musclesJSON = JSON.stringify(muscles);
const movementsJSON = JSON.stringify(movements);

const fs = require('fs');

fs.writeFile('./scripts/data/muscles.json', musclesJSON, (err) => {
  if(err) throw err;
  console.log('muscles.json written!');
});
fs.writeFile('./scripts/data/movements.json', movementsJSON, (err) => {
  if(err) throw err;
  console.log('movements.json written!');
});
